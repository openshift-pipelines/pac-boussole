...
if not match:
    print(
        f"⚠️ No valid command found in comment: {PAC_TRIGGER_COMMENT}", file=sys.stderr
    )
    sys.exit(1)
...
if command == "assign":
    ...
    ressponse = help_command()

if response and response.status_code in [200, 201, 204]:
    print(f"✅ Successfully processed {command}: {', '.join(values) if values else ''}")
else:
    print(
        f"❌ Failed to process {command}: {response.status_code if response else 'N/A'} - {response.text if response else 'N/A'}",
        file=sys.stderr,
    )
    sys.exit(1)
