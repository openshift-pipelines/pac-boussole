...
generate:
	@echo "Generating $(PIPELINE_PROW) 🚿" && \
		...
		$(UVCMD) ./hack/tekton-task-embed-script.py code/base.yaml | prettier --parser=yaml >> $(PIPELINE_PROW)
...
check:
	@tmpfile=`mktemp /tmp/check-XXXXXX`; \
		...
		echo "Changes is needed: make sure to modify the prun in code/ and run make generate before pushing"; \
